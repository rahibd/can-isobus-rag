...
behind published ISOBUS communication research
...
question time.
Everything runs locally through LM Studio — no API
...
Unsloth, targeting a full 7-8B model
...
Setup
RAG pipeline
Install LM Studio, load a local model (e.g.
`gemma4:e4b`), and start its Local Server (`http://localhost:1234`).
`pip install -r requirements.txt`
Drop your own `.txt` / `.pdf` / `.docx` files into `sample_docs/`.
...
Fine-tuning experiment
`python dataset_prep.py` — generates `finetune_data.jsonl` from your
...
Upload `colab_finetune.ipynb` to Google Colab,
set `Runtime → Change runtime type → T4 GPU`, and run all cells. Trains
a full 7-8B model via Unsloth
...
GitHub: github.com/rahibd
LinkedIn: linkedin.com/in/mbmbd
Blog: mmotalab.medium.com
License
MIT — see `LICENSE`.
